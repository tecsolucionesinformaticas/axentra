...
hbb_name = ('Axentra' if windows else 'axentra') + ('.exe' if windows else '')
...
def generate_control_file(version):
    ...
    content = """Package: axentra
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: TEC Soluciones Informaticas <soporte@tecsi.com.ar>
Homepage: https://axentra.tecsi.com.ar
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/axentra')
    system2('mkdir -p tmpdeb/etc/axentra/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/axentra/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/axentra || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/axentra/')
    system2(
        'cp ../res/rustdesk.service tmpdeb/usr/share/axentra/files/systemd/axentra.service')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/axentra.png')
    system2(
    ...
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/axentra/')
    system2(
        'cp ../res/pam.d/rustdesk.debian tmpdeb/etc/pam.d/axentra')
    system2(
    ...
    system2('dpkg-deb -b tmpdeb axentra.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('axentra.deb', '../axentra-%s.deb' % version)
    os.chdir("..")
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/axentra || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/axentra/')
    system2(
        'cp ../res/rustdesk.service tmpdeb/usr/share/axentra/files/systemd/axentra.service')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/axentra.png')
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/Axentra.exe')
    os.chdir('../..')
    ...
    os.rename('./rustdesk_portable.exe', f'./Axentra-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/Axentra-{version}-install.exe')
...
def main():
    ...
    '''.format(pa, version))
                ...
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                    'cp res/rustdesk.service tmpdeb/usr/share/axentra/files/systemd/')
                system2(
                    'cp res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/axentra.png')
                system2(
                    'cp res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/axentra.svg')
                ...
                system2('strip tmpdeb/usr/bin/axentra')
                system2('mkdir -p tmpdeb/usr/share/axentra')
                system2('mv tmpdeb/usr/bin/axentra tmpdeb/usr/share/axentra/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/axentra/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb axentra.deb; /bin/rm -rf tmpdeb/')
